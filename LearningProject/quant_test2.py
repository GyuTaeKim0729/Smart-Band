...
def run_quant_inference(wanted_words, sample_rate, clip_duration_ms,
    ...
    all_test_fingerprints = load_X(X_test_signals_paths)
    all_test_ground_truth = load_y(y_test_path)
    all_test_ground_truth = np_utils.to_categorical(all_test_ground_truth)
    ...
    logits, debugs = models.create_model(
        fingerprint_input,
        model_settings,
    ...
    '''
    '''
    set_size = int(all_train_fingerprints.shape[0])

    tf.logging.info('set_size=%d', set_size)
    total_accuracy = 0
    total_conf_matrix = None
    for i in xrange(FLAGS.batch_size, all_train_fingerprints.shape[0], FLAGS.batch_size):
        training_fingerprints = all_train_fingerprints[i - FLAGS.batch_size:i]
        training_ground_truth = all_train_ground_truth[i - FLAGS.batch_size:i]
        ...
        total_accuracy += (training_accuracy * FLAGS.batch_size) / all_train_fingerprints.shape[0]
    ...
    testfp = []
    testgt = []
    testfp.append(test_fingerprints[-1])
    testgt.append(test_ground_truth[-1])
    print(np.trunc(np.array(testfp) * 2))
'''    
    pr = sess.run(debugs,
        feed_dict={
            fingerprint_input: testfp,
            ground_truth_input: testgt,
        })
    for pre in pr:
        for p in pre:
            print("")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--checkpoint',
        type=str,
        default='model/best/ds_cnn_9375_x.ckpt-10_bnfused',
        help='Checkpoint to load the weights from.')
    parser.add_argument(
        '--model_size_info',
        type=int,
        nargs="+",
        default=[5, 32, 6, 6, 2, 1, 16, 6, 6, 1, 1, 16, 6, 6, 1, 1, 16, 6, 6, 1, 1, 16, 6, 6, 1, 1],
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=[64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64],
